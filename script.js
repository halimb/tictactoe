...
function checkState(board) {
    ...
    if (res == NT) {
        //check for tie state
        loop: for (var i = 0; i < board.length; i++) {
...
function displayState(state) {
    if (state != TIE) {
        var winner = state == 1 ? "X" :
            "O"
        var message = winner + "<br><span class='txt'>WINS</span>";
    } else {
        var message = "<div class='tie'>TIE</div>";
    }
...
function displayTerminal(state) {
    ...
    setTimeout(function() { displayState(state); }, 1500)
